...
**Is this a gap buffer?** <br>

No. A gap buffer keeps a movable gap near the cursor for text editing. The Shift-To-Middle Array is a general-purpose deque alternative that maintains space at both ends and shifts data only during resizes. It’s optimized for fast insertions at both head and tail, not at a single point. <br>

**Is this like a free list?**

Not really. A free list is a memory management structure that tracks blocks of unused memory, often used in allocators or object pools. The Shift-To-Middle Array, by contrast, does not manage memory at the level of arbitrary free blocks. Instead, it maintains a contiguous array with reserved capacity at both ends to allow efficient insertions. There's no linked list of free nodes or reuse of freed memory blocks—the "free space" in this structure is simply preallocated slack at the head and tail, not dynamically reclaimed fragments. <br>

| Feature                           | Gap Buffer                        | Free List                          | Shift-To-Middle Array                     |
| --------------------------------- | --------------------------------- | ---------------------------------- | ----------------------------------------- |
| **Primary Use Case**              | Text editors (cursor-based edits) | Memory management / object pooling | General-purpose dynamic deque             |
| **Structure Type**                | Contiguous array + movable gap    | Linked list of free blocks         | Contiguous array with slack on both ends  |
| **Insert/Delete Complexity**      | O(1) near gap, O(n) elsewhere     | O(1) allocation, varies on reuse   | Amortized O(1) at head/tail               |
| **Supports Middle Insertions**    | Efficient near cursor             | Not applicable                     | Less efficient, requires shifting         |
| **Handles Arbitrary Free Blocks** | No                                | Yes                                | No                                        |
| **Data Shifting Strategy**        | Frequent, to move the gap         | None                               | Rare, only on resize to recenter data     |
| **Memory Fragmentation Handling** | No                                | Yes                                | No (uses a contiguous block)              |
| **Allocation Pattern**            | One gap, moves with cursor        | Multiple reused nodes              | Preallocates and resizes as needed        |
| **Ideal For**                     | Editable text buffers             | Custom allocators, game engines    | Queues, double-ended buffers, general use |
